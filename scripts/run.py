...
def _write_findings(result, labels):
    ...
    # Conclusions — the decision table answering the research question. Verdicts
    # are stable given the n=92 results; written as static prose.
    lines += [
        "## Conclusions — which shrine properties can a local LLM safely auto-suggest?",
        "",
        "Granularity-aware precision is the relevant number for a maintainer "
        "(crediting a correct coarser answer). Verdicts from the n=92 run:",
        "",
        ...
        "",
        "**Answering the question:** a local LLM (Gemma `gemma3:12b`) is a useful "
        "gap-filler for **categorical / near-constant** shrine facts when credited "
        "at the right granularity, and it **appropriately abstains** on specific "
        "values (dates, coordinates) rather than hallucinating — so its *errors* "
        "are rare, but so is its *coverage* of the hard fields. Three takeaways:",
        "",
        "1. **Self-verification did not help — it hurt** (precision 0.58→0.55 at "
        "n=92, replicating the n=42 result). Do not add a naive verify pass.",
        "2. **Popularity didn't matter and there is no memorization signal** "
        "(flat/again-better on the no-Wikipedia tail) — the model leans on "
        "structural priors (\"a shrine is in Japan, is Shinto, sits in *some* "
        "region\"), not entity-specific recall.",
        "3. **Granularity is the real story for entity properties**: strict "
        "exact-QID scoring badly understates a model that is reliably right at a "
        "coarser level.",
        "",
        "*Scope: one curated domain, a 12B local model, n=92 — indicative, not a "
        "settled benchmark. The pipeline (`scripts/run.py`) reproduces every "
        "number.*",
        "",
